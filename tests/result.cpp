...
constexpr std::array ASSETS_PATH = {
    "assets/1mb.wav",         "assets/5mb.wav",      "assets/10mb.wav",
    "assets/the_bidding.wav", "assets/long_wav.wav",
};
...
public:
  ...
  static constexpr T COMPLEX_ERROR = static_cast<T>(1e-1);
  ...
  auto get_map(const path &song_name) -> AudioProcessingData {
    ...
    static const std::unordered_map<std::string, AudioProcessingData>
        SONGS_DATA = {
            ...
            {"assets/10mb.wav",
             AudioProcessingData(
                 ...
                 complex<T>{52.28627384948578 + 0.01103484824364466i},
                 {1025, 5149}, static_cast<T>(1235600.6060180683), {1025, 5149},
                 static_cast<T>(-196225434.62754357))},
            // the_bidding.wav
            {"assets/the_bidding.wav",
             AudioProcessingData(
                 7084032, static_cast<T>(-2169.4102478027344), {1025, 13837},
                 complex<T>{-2169.388488769531 - 2.4889922962644515e-12i},
                 {1025, 13837}, static_cast<T>(23341901.69158635),
                 {1025, 13837}, static_cast<T>(-196082315.69200513))},
            // long_wav.wav
            {"assets/long_wav.wav",
             AudioProcessingData(
                 18113536, static_cast<T>(1163.124267578125), {1025, 35379},
                 complex<T>{1163.1448364257758 + 2.879711836285999e-12i},
                 {1025, 35379}, static_cast<T>(32961517.675204396),
                 {1025, 35379}, static_cast<T>(-733725741.707634))}};
...
TYPED_TEST(ResultsTest, ValidSpectrogram) {
  ...
  for (const auto &song : ASSETS_PATH) {
    ...
    // validate sum
    ASSERT_NEAR(data.spectrogram_sum, sum_vector(spectrogram),
                TestFixture::COMPLEX_ERROR);
  }
}
...
TYPED_TEST(ResultsTest, ValidNormalizedSpectrogram) {
  ...
  for (const auto &song : ASSETS_PATH) {
    ...
    ASSERT_NEAR(data.normalized_spectrogram_sum,
                sum_vector(normalized_spectrogram), TestFixture::COMPLEX_ERROR);
  }
}
